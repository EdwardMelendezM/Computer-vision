import cv2
image_3chan = cv2.imread('src/nut_bolt.png')
...
contours_list, hierarchy = cv2.findContours(
    binary_im, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
for i in range(0, len(contours_list)):
    ...
    if contour_info[2] == -1 and contour_info[3] == -1:
        with_contours = cv2.drawContours(
            image_3chan_copy, contours_list, i, [0, 255, 0], thickness=3)
        print('Se detecta el contorno del perno')
    if contour_info[2] == -1 and contour_info[3] != -1:
        with_contours = cv2.drawContours(
            with_contours, contours_list, i, [0, 0, 255], thickness=3)
